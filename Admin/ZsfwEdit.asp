...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="5" cellspacing="1">
  <form name="editZsfwForm" method="post" action="ZsfwEdit.asp?Action=SaveEdit&Result=<%=Result%>&ID=<%=ID%>">
    <tr>
      <th height="22" colspan="2" sytle="line-height:150%">【<%If Result = "Add" then%>添加<%ElseIf Result = "Modify" then%>修改<%End If%>展商服务】</th>
    </tr>
    <tr>
      <td width="20%" align="right" class="forumRow">服务标题：</td>
...
sub ZsfwEdit()
  ...
  if Action="SaveEdit" then
    ...
    GroupIdName=split(Request.Form("GroupID"),"┎╂┚")
	  ...
	  set rs=Nothing
	  set rs=server.createobject("adodb.recordset")
	  sql="select top 1 ID from Qianbo_Zsfw order by ID desc"
	  rs.open sql,conn,1,1
	  ID=rs("ID")
	  rs.close
	  set rs=Nothing
	  if ISHTML = 1 then
	  call htmll("","",""&ZsfwNameDiy&""&Separated&""&ID&"."&HTMLName&"","Zsfw.asp","ID=",ID,"","")
	  End If
	end if
    ...
    if ISHTML = 1 then
    response.write "<script language='javascript'>alert('设置成功，相关静态页面已更新！');location.replace('ZsfwList.asp');</script>"
	Else
	response.write "<script language='javascript'>alert('设置成功！');location.replace('ZsfwList.asp');</script>"
	End If
  else
	if Result="Modify" then
      set rs = server.createobject("adodb.recordset")
