...
<%
if Instr(session("AdminPurview"),"|14,")=0 then
...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From Qianbo_DownSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from  Qianbo_DownSort  where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from  Qianbo_Download where Instr(SortPath,'"&SortPath&"')>0")
    response.write ("<script language='javascript'>alert('成功删除本类、子类及所有下属信息条目！');location.replace('DownSort.asp');</script>")
  Case "Save"
	saveFolder ()
  Case "Edit"
	editFolder
...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="5" cellspacing="1">
  <tr>
    <th height="22" sytle="line-height:150%">【管理下载类别】</th>
  </tr>
  <tr>
    <td align="center" nowrap class="forumRow"><a href="DownSort.asp?Action=Add&ParentID=0">添加一级分类</a> | <a href="DownList.asp">查看所有下载</a></td>
  </tr>
  <tr>
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From Qianbo_DownSort where ParentID="&id&" order by id"
  rs.open sql,conn,1,1
  ...
  while not rs.eof
    ChildCount=conn.execute("select count(*) from Qianbo_DownSort where ParentID="&rs("id"))(0)
    ...
    response.write("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>操作：</font><a href='DownSort.asp?Action=Add&ParentID="&rs("id")&"'>添加</a>")
    response.write(" | <a href='DownSort.asp?Action=Edit&ID="&rs("id")&"'>修改</a>")
    response.write(" | <a href='DownSort.asp?Action=Move&ID="&rs("id")&"&ParentID="&rs("Parentid")&"&SortName="&rs("SortName")&"&SortPath="&rs("SortPath")&"'>移</a>")
	...
	response.write(" | <a href=javascript:ConfirmDelSort('DownSort',"&rs("id")&")>删除</a>")
    response.write("&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>下载：</font><a href='DownEdit.asp?Result=Add'>添加</a>")
    response.write(" | <a href='DownList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"'>列表</a>")
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From Qianbo_DownSort where ID="&ParentID
    rs.open sql,conn,1,1
...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="5" cellspacing="1">
  <form name="FolderForm" method="post" action="DownSort.asp?Action=Save&From=Add">
    <tr>
      <th height="22" sytle="line-height:150%">【添加下载类别】</th>
    </tr>
    <tr>
...
Function saveFolder
  ...
  Set rs=server.CreateObject("adodb.recordset")
  if From="Add" then
    sql="Select * From Qianbo_DownSort"
  ...
  rs.update
  response.write ("<script language='javascript'>alert('"&Action&"成功！');location.replace('DownSort.asp');</script>")
End Function
...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="5" cellspacing="1">
  <form name="FolderForm" method="post" action="DownSort.asp?Action=Save&From=Edit&ID=<%=ID%>">
    <tr>
      <th height="22" sytle="line-height:150%">【修改下载类别】</th>
    </tr>
    <tr>
      <td class="forumRow">| 根类 →
        <% if ParentID<>0 then TextPath(ParentID)%></td>
...
<table id="SortFromTo" class="tableBorder" width="95%" border="0" align="center" cellpadding="5" cellspacing="1">
  <form name="MoveForm" method="post" action="DownSort.asp?Action=MoveSave">
    <tr>
      <th height="22" sytle="line-height:150%">【移动下载类别】</th>
    </tr>
    <tr>
      <td class="forumRow">→
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From Qianbo_DownSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update Qianbo_DownSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
  conn.execute("update Qianbo_DownSort set ParentID='"&toID&"' where ID="&fromID)
  conn.execute("update Qianbo_Download set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")
  response.write ("<script language='javascript'>alert('下载类别移动成功！');location.replace('DownSort.asp');</script>")
End Function
%>