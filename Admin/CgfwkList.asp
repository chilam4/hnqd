...
<form action="DelContent.asp?Result=Cgfw" method="post" name="formDel">
  <tr>
...
function CgfwList()
  ...
  if(idcount>0 and sqlid<>"") then
    ...
    while(not rs.eof)
      ...
      Response.Write "<td nowrap class=""forumRow""><a href='CgfwEdit.asp?Result=Modify&ID="&rs("ID")&"' title="""&rs("AboutName")&""">"&StrLeft(rs("AboutName"),28)&"</a>"& vbCrLf
      ...
      Response.Write "<td align=""center""nowrap class=""forumRow""><a href='CgfwEdit.asp?Result=Add'>添加</a> <a href='CgfwEdit.asp?Result=Modify&ID="&rs("ID")&"'>修改</a> <a href='CgfwList.asp?Result=ModifySequence&ID="&rs("ID")&"'>排序</a></td>" & vbCrLf
...
sub ModifySequence()
  ...
  set rs = server.createobject("adodb.recordset")
  sql="select * from Qianbo_Cgfw where ID="& ID
  rs.open sql,conn,1,1
  ...
  response.write "<form action='CgfwList.asp?Result=SaveSequence' method='post' name='formSequence'>"
  response.write "<tr>"
...
sub SaveSequence()
  dim rs,sql
  set rs = server.createobject("adodb.recordset")
  sql="select * from Qianbo_Cgfw where ID="& request.form("ID")
  rs.open sql,conn,1,3
  rs("Sequence")=request.form("Sequence")
  rs.update
  rs.close
  set rs=nothing
  response.redirect "CgfwList.asp"
end sub
%>