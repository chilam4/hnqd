...
<table class="tableBorder" width="95%" border="0" align="center" cellpadding="5" cellspacing="1">
  <form name="editCgfwForm" method="post" action="CgfwEdit.asp?Action=SaveEdit&Result=<%=Result%>&ID=<%=ID%>">
    <tr>
...
sub CgfwEdit()
  ...
  if Action="SaveEdit" then
    ...
    GroupIdName=split(Request.Form("GroupID"),"┎╂┚")
	  ...
	  set rs=Nothing
	  set rs=server.createobject("adodb.recordset")
	  sql="select top 1 ID from Qianbo_Cgfw order by ID desc"
	  rs.open sql,conn,1,1
	  ID=rs("ID")
	  rs.close
	  set rs=Nothing
	  if ISHTML = 1 then
	  call htmll("","",""&CgfwNameDiy&""&Separated&""&ID&"."&HTMLName&"","Cgfw.asp","ID=",ID,"","")
	  End If
	end if
    ...
    if ISHTML = 1 then
    response.write "<script language='javascript'>alert('设置成功，相关静态页面已更新！');location.replace('CgfwList.asp');</script>"
	Else
	response.write "<script language='javascript'>alert('设置成功！');location.replace('CgfwList.asp');</script>"
	End If
  else
	if Result="Modify" then
      set rs = server.createobject("adodb.recordset")
