...
<%
if Instr(session("AdminPurview"),"|7,")=0 then
...
function PlaceFlag()
  if Result="Search" Then
	If Keyword<>"" Then
		Response.Write "新闻：列表 -> 检索 -> 关键字：<font color='red'>"&Keyword&"</font>"
	Else
		Response.Write "新闻：列表 -> 检索 -> 关键字为空(显示全部新闻)"
	End If
  else
    if SortPath<>"" then
...
<form name="formSearch" method="post" action="Search.asp?Result=News">
  <tr>
    <th height="22" sytle="line-height:150%">【新闻检索及分类查看】</th>
  </tr>
  <tr>
    <td class="forumRow">关键字：<input name="Keyword" type="text" value="<%=Keyword%>" size="20"> <input name="submitSearch" type="submit" value="搜索新闻"></td>
  </tr>
  <tr>
  ...
  if(idcount>0 and sqlid<>"") then
    ...
    while(not rs.eof)
	  ...
	  if StrLen((rs("NewsName")))>40 then
        Response.Write "<td nowrap title='"&rs("NewsName")&"' class=""forumRow""><a href='NewsEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&StrLeft(rs("NewsName"),37)&"</a></td>" & vbCrLf
      else
        Response.Write "<td nowrap title='"&rs("NewsName")&"' class=""forumRow""><a href='NewsEdit.asp?Result=Modify&ID="&rs("ID")&"''>"&rs("NewsName")&"</a></td>" & vbCrLf
      end if 
      Response.Write "<td nowrap align=""center"" class=""forumRow"">" & vbCrLf
      if rs("NoticeFlag") Then
      Response.Write "<font color='red'>公告</font>"
	  Else
	  Response.Write "<font color='red'>新闻</font>"
	  End If
	  Response.Write "</td>"
	  ...
	  if StrLen((rs("Source")))>13 then
        Response.Write "<td nowrap title='来源：&#13;"&rs("Source")&"' class=""forumRow"">"&StrLeft(rs("Source"),10)&"</td>" & vbCrLf
      else
        Response.Write "<td nowrap title='来源：&#13;"&rs("Source")&"' class=""forumRow"">"&rs("Source")&"</td>" & vbCrLf
      end if 
      Response.Write "<td nowrap class=""forumRow"">"&rs("AddTime")&"</td>" & vbCrLf
      Response.Write "<td align=""center""nowrap class=""forumRow""><a href='NewsEdit.asp?Result=Add'>添加</a> <a href='NewsEdit.asp?Result=Modify&ID="&rs("ID")&"'>修改</a></td>" & vbCrLf
    ...
    response.write "<tr><td nowrap align='center' colspan='9' class=""forumRow"">暂无企业新闻</td></tr>"
  end if
  Response.Write "<tr>" & vbCrLf
